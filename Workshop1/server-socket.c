...
void *handle_client(void *arg) {  // Threaded function.
    ...
    while (1) {
        ...
        if (bytes_read == 0) {
            ...
            for (int i = 0; i < bytes_read; i++) { // Modify message (ALL CAPS).
...
int main(void) {
    ...
    sa.sin_addr.s_addr = htonl(INADDR_ANY); // Accept incoming connections from any IP address.
    ...
    int opt = 1; // Setup so the same port can be used inmediately.
    ...
    while (1) {
        ...
        int *client_fd = malloc(sizeof(int)); // Assign memory for the desciptor.
        *client_fd = accept(socket_fd, (struct sockaddr *)&client_addr, &addr_size);
        ...
        pthread_detach(thread_id); // Free the resources used by the thread.
    }
    close(socket_fd);
    return (0);
}
